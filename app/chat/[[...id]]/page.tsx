...
export default function ChatPage() {
  ...
  const [currentConvId, setCurrentConvId] = useState<string | null>(urlConvId);
  ...
  useEffect(() => {
    if (session?.user) {
      fetchConversations().then((saved) => {
        ...
        if (urlConvId) {
          const match = saved.find((c) => c.id === urlConvId);
          setCurrentConvId(match ? match.id : saved[0]?.id ?? null);
  ...
  const handleDeleteConversation = async (id: string) => {
    ...
    if (currentConvId === id) {
      ...
      selectConv(remaining.length > 0 ? remaining[0].id : null);
    }
  };
